...
/* === ÉVOLUTION SUR DASHBOARD === */
function renderDashDefis() {
  const lst = document.getElementById('defiEvoList');
  const card = document.getElementById('defiEvoDash');
  if (!lst) return;
  ...
  if (!keys.length) {
    lst.innerHTML = '<div class="empty">Commencez un défi pour voir votre progression ici.</div>';
    return;
  }
  ...
  lst.innerHTML = keys.map(i => {
    ...
    const elapsed = Math.floor((new Date()-new Date(joined[i]))/(1000*60*60*24));
    const done = pct >= 100;
    return `<div class="dash-evo-item">
      <div class="dei-header">
        <div class="dei-name">${d.name}</div>
        <div class="dei-pct ${done?'done':''}">${done?'✓ Terminé':pct+'%'}</div>
      </div>
      <div class="dei-bar">
        <div class="dei-fill" style="width:${pct}%"></div>
      </div>
      <div class="dei-meta">
        <span>${fmt(saved,cur)} économisé</span>
        <span>Objectif : ${fmt(d.obj,cur)}</span>
        <span>Restant : ${fmt(Math.max(0,d.obj-saved),cur)}</span>
      </div>
      <div class="dei-day">Jour ${elapsed} — ${d.dur}</div>
    </div>`;
  }).join('');
}
...
function renderDashGoals() {
  ...
  if (!goals.length) {
    lst.innerHTML = '<div class="empty">Créez un objectif pour suivre votre progression.</div>';
    return;
  }

  lst.innerHTML = goals.map(g => {
    const pct = Math.min(100, Math.round((g.saved/g.target)*100))||0;
    const restant = Math.max(0, g.target - g.saved);
    const done = pct >= 100;
    const dt = g.date ? new Date(g.date).toLocaleDateString('fr-FR',{day:'numeric',month:'long',year:'numeric'}) : '';
    return `<div class="dash-evo-item">
      <div class="dei-header">
        <div class="dei-name">${g.name}</div>
        <div class="dei-pct ${done?'done':''}">${done?'✓ Atteint !':pct+'%'}</div>
      </div>
      <div class="dei-bar">
        <div class="dei-fill goal-fill" style="width:${pct}%"></div>
      </div>
      <div class="dei-meta">
        <span>${fmt(g.saved,cur)} économisé</span>
        <span>Objectif : ${fmt(g.target,cur)}</span>
        <span>Restant : ${fmt(restant,cur)}</span>
      </div>
      ${dt?'<div class="dei-day">Date cible : '+dt+'</div>':''}
    </div>`;
  }).join('');
}
...
function notifInit() {
  ...
  if (Notification.permission === 'default') {
    Notification.requestPermission().then(p => {
      if (p === 'granted') {
        toast('Notifications activées ! Vous recevrez des rappels quotidiens.');
        scheduleNotif();
      }
    });
...
function scheduleNotif() {
  ...
  const lastNotif = localStorage.getItem('bs_notif');
  ...
  // Notification défis
  const defiKeys = Object.keys(joined);
  if (defiKeys.length > 0) {
    setTimeout(() => {
      const msgs = defiKeys.map(i => {
        const d = DEFIS[parseInt(i)]; if (!d) return null;
        const saved = savings[i]||0;
        const pct = Math.min(100, Math.round((saved/d.obj)*100));
        return d.name + ' — ' + pct + '% atteint';
      }).filter(Boolean);
      new Notification('BudgetSmart — Vos défis vous attendent !', {
        body: msgs.join('\n') + '\nAjoutez votre montant du jour pour progresser.',
  ...
  // Notification objectifs
  if (goals.length > 0) {
    setTimeout(() => {
      const p = ld('profile', {}), cur = p.currency||'€';
      const msgs = goals.map(g => {
        const pct = Math.min(100, Math.round((g.saved/g.target)*100))||0;
        const restant = Math.max(0, g.target - g.saved);
        return g.name + ' — ' + pct + '% · Restant : ' + restant.toFixed(0) + ' ' + cur;
      });
      new Notification('BudgetSmart — Vos objectifs d\'épargne', {
        body: msgs.join('\n') + '\nN\'oubliez pas d\'ajouter vos économies !',
        icon: './icons/icon-192.png',
        badge: './icons/icon-72.png',
        tag: 'goals'
      });
  ...
  // Notification dépenses - rappel bilan journalier
  const todayEntries = entries.filter(e => e.date === today);
  if (goals.length > 0 || defiKeys.length > 0) {
    setTimeout(() => {
      const hasTodayEntry = todayEntries.length > 0;
      new Notification('BudgetSmart — ' + (hasTodayEntry ? 'Bilan du jour' : 'Rappel quotidien'), {
        body: hasTodayEntry
          ? 'Vous avez enregistré ' + todayEntries.length + ' entrée(s) aujourd\'hui. Continuez comme ça !'
          : 'N\'oubliez pas d\'enregistrer vos dépenses et revenus d\'aujourd\'hui !',
        icon: './icons/icon-192.png',
        badge: './icons/icon-72.png',
        tag: 'daily'
      });
